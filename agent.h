...
protected:
    ...
    //std::vector<std::vector<weight>> netnet;
    //std::vector<std::vector<std::vector<std::vector<double> > > > TableA = make_4d_vector<double>(pow(10,6), 4, 4,3, -1);
    //std::vector<std::vector<std::vector<weight> > > TableB = make_3d_vector<weight>(pow(10,6), 3, 3, -1);
...
public:
    ...
    if (Vmax(0)>=7 & IFBonus(engine)==1)
    //std::cout<<(N_bonustile+1)/(N_totaltile+1)<<"\n";
    //if (Vmax(0)>=7 & (((N_bonustile+1)/(N_totaltile+1)) <= 0.04))
    //if (Vmax(0)>=7)
       { 
             std::uniform_int_distribution<int> bonustile(4,4+(Vmax(0)-7));
